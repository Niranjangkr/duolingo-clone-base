...
import { CourseDataType, Quiz as QuizType, Unit, UserCourseProgressType } from "@/types";
...
type QuizProps = {
  ...
  progress: UserCourseProgressType,
};
...
export const Quiz = ({
  ...
  const onContinue = () => {
    ...
    if (correctOption === selectedOption) {
      startTransition(async() => {
        ...
        if(lastQuestionCompleted && level !== "advanced"){
          questionIndex = 0;
        ...
        console.log("LEVEL QINDEX ", level, questionIndex);
        await upsertCourseProgress(userProgress.id, level, questionIndex);
  ...
  useEffect(() => {
    if(totalCompletedQuestions <= currentCourseContent.length){
      setTitle(currentCourseContent[userProgress.questionIndex]?.question)
    }
